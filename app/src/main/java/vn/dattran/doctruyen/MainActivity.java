...
public class MainActivity extends Activity implements TextToSpeech.OnInitListener {
  ...
  @Override public void onCreate(Bundle b){super.onCreate(b);
    ...
    WebSettings s=web.getSettings();s.setJavaScriptEnabled(true);s.setDomStorageEnabled(true);s.setDatabaseEnabled(true);s.setBuiltInZoomControls(true);s.setDisplayZoomControls(false);s.setMixedContentMode(WebSettings.MIXED_CONTENT_COMPATIBILITY_MODE);
    // Giữ nguyên User-Agent chuẩn của Android WebView. SangTacViet có thể báo 4005
    // nếu phát hiện chuỗi nhận dạng lạ được nối thêm vào User-Agent.
    CookieManager cookies=CookieManager.getInstance();cookies.setAcceptCookie(true);cookies.setAcceptThirdPartyCookies(web,true);
    web.addJavascriptInterface(new Bridge(),"AndroidReader");web.setWebChromeClient(new WebChromeClient(){
      @Override public boolean onJsAlert(WebView view,String url,String message,JsResult result){
        if(url!=null&&url.contains("sangtacviet")&&message!=null&&message.contains("4005")){result.confirm();return true;}
        return super.onJsAlert(view,url,message,result);
      }
    });web.setWebViewClient(new WebViewClient(){@Override public void onPageFinished(WebView v,String u){cookies.flush();address.setText(u);v.evaluateJavascript(readerJs+";DocReader.collect()",x->{});if(reading&&loadingNext){loadingNext=false;v.postDelayed(()->start(0),900);}}});
